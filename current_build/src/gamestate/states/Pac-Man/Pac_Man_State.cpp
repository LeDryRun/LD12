...
Pac_Man_State::Pac_Man_State(Imagehandler& imagehandler,Audiohandler& audiohandler):core_layer("core_layer"){
	tilemap.load_from_file("../assets/Pac-Man/maps/map1.map");
	...
	for(int i=0;i<(int)ghosts.size();i++){
		...
		ghosts.at(i).create_hitbox(Point(32,32),Point(0,0));
	...
	pac_man.load_sounds(audiohandler);
	core_layer.set_original_size(1920,1080);

	mat.create(Point(50,50));
}
...
void Pac_Man_State::load_sprites(Imagehandler& imagehandler){
	...
	mat.add_animation("Mat_Wright");
	mat.load_animations(imagehandler);
	mat.ref_animation(0).set_looping(true);
...
void Pac_Man_State::update(Mousey& mouse, Keyblade& keyboard, Gamepad& gamepad){
	Duration_Check::start("-PAC update");
	Gamestate::update_gui_layer(mouse,keyboard,gamepad);
	if(sound==false){
		sound=true;
		tilemap.find_path(Point(1,1),Point(1,1));
	}
	...
	*/
	...
	Duration_Check::stop("-PAC update");
}



void Pac_Man_State::render(sf::RenderWindow& window){
	Duration_Check::start("-PAC render");
	...
	*/
	...
	for(unsigned int i=0;i<ghosts.size();i++){
		...
		ghosts.at(i).animate();
		window.draw(ghosts.at(i));
	}


	//window.draw(mat.ref_current_animation().animate());
	mat.animate();
	window.draw(mat);

	Gamestate::render_gui_layer(window);
	Duration_Check::stop("-PAC render");
}
